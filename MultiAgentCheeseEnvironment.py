...
class CheeseEnv(MultiAgentEnv):
    ...
    def __init__(self, config=EnvContext):
        ...
        self.demand_max = np.array([self.production_levels[p][-1] for p in self.products])
        ...
        self.multi_agent_setting = config["multi_agent_setting"] if "multi_agent_setting" in config else "multi_group"
        print("MULTI AGENT SETTING: ", self.multi_agent_setting)
        
        if self.multi_agent_setting == "multi_all":
                    # create purchase and issuance agents for each product
            self.num_agents = 1 + self.numAges - 1  # purchase agent + issuance agents for each age class except the last one
            # list of all agent ids -> ["purchase_agent", "issuance_agent_1", "issuance_agent_2" ...] 
            self._agent_ids = ["purchase_agent"] + [f"issuance_agent_{n}" for n in self.issuance_ages[:-1]]
        elif self.multi_agent_setting == "multi_group" or self.multi_agent_setting == "multi_production":
            self.num_agents = 1 + self.nProducts  # purchase agent + issuance agents for each product
            self._agent_ids = ["purchase_agent"] + [f"production_agent_{p}" for p in self.products]
        else:
            self.num_agents = 1
            self._agent_ids = ["purchase_agent"]
    
        self._ordered_agent_ids = deepcopy(self._agent_ids)
        if self.use_issuance_model:
            ...
            elif self.multi_agent_setting == "multi_all" or self.multi_agent_setting == "multi_production":
                self.action_space = spaces.Dict({agent_id: spaces.Box(low=-1.0, high=1.0, shape=(1,)) for agent_id in self._agent_ids})
            elif self.multi_agent_setting == "multi_group":
                action_space_dict = {"purchase_agent": spaces.Box(low=-1.0, high=1.0, shape=(1,))}
                for p in self.products:
                    action_space_dict[f"production_agent_{p}"] = spaces.Box(low=np.full((len(self.ageRange[p]),),-1.0), high=np.full((len(self.ageRange[p]),),1.0), shape=(len(self.ageRange[p]),))
                self.action_space = spaces.Dict(action_space_dict)
                
        self._action_space_in_preferred_format = True
        self._obs_space_in_preferred_format = True
        if self.num_agents > 1:
            # all policies will be trained -> keys = agent_ids
            policies = self._agent_ids
            policies_to_train = self._agent_ids

            # map policies to agents -> each agent id is the same as policy id
            def policies_mapping_fn(agent_id):
                return agent_id

            self.multiagent = {"policies": policies, "policy_mapping_fn": policies_mapping_fn, "policies_to_train": policies_to_train}

        if self.multi_agent_setting == "multi_production":
            original_space = {agent_id: spaces.Dict({
        ...
        original_space = spaces.Dict(original_space)
    ...
    def get_heuristic_action(self):
        ...
        issuance = [self.heuristic_model.getVarByName(f"iss[{a},0]").X for a in self.issuance_ages]
        return purchasing, issuance
    
    #internal getter for state
    def  _get_obs(self):
        if self.multi_agent_setting == "multi_production":
            return {agent_id: {"prices": np.array([self.priceDistributions[p].cdf(self.prices[p]) for p in self.processes]), "inventory": self.inventory_position/self.maxInventory, "max_production": self.max_production/self.demand_max} for agent_id in self._agent_ids}
        else:
            return {agent_id: {"prices": np.array([self.priceDistributions[p].cdf(self.prices[p]) for p in self.processes]), "inventory": self.inventory_position/self.maxInventory} for agent_id in self._agent_ids}

    def _get_state_from_obs(self, obs):
        first_key = list(obs.keys())[0]
        self.inventory_position = obs[first_key]["inventory"] * self.maxInventory
        self.prices = [self.priceDistributions[p].ppf(obs[first_key]["prices"][p]) for p in self.processes]
    ...
    def step_continuous(self, action):
       ...
       #map neural network output to implementable action
            ...
            #flatten action for multi-agent settings
            if self.num_agents > 1:
                action = np.concatenate([action[agent_id] for agent_id in self._ordered_agent_ids])
            ...
            if self.multi_agent_setting == "multi_production":
                ...
                if self.avoid_outdating:
                    issuance = {i: ((action[1+i-self.issuance_ages[0]]+1)/2) * self.inventory_position[i] for i in self.issuance_ages[:-1]}
                    issuance[self.numAges-1] = self.inventory_position[self.numAges-1]
                else:
                    issuance = {i: ((action[1+i-self.issuance_ages[0]]+1)/2) * self.inventory_position[i] for i in self.issuance_ages}
                for i in self.ages:
                    if i not in self.issuance_ages:
                        issuance[i] = 0
        ...
        norm_reward = {agent_id: norm_reward for agent_id in self._agent_ids}

        termination_dict = {agent_id: False for agent_id in self._agent_ids}
        termination_dict["__all__"] = False
        truncation_dict = {agent_id: self.n_steps == self.max_horizon for agent_id in self._agent_ids}
        truncation_dict["__all__"] = self.n_steps == self.max_horizon
            
        return observation, norm_reward, termination_dict, truncation_dict, {"purchase_agent": {"revenue":revenue, "purchasing_cost": purchasing_cost, "holding_cost":holding_cost, "purchasing":purchasing, "production":production, "issuance":[issuance[a] for a in self.ages],  "outdating": outdating, "inventory": self.inventory_position, "prices":prev_prices}}
    ...
    def sample_prices(self):
        ...
        if self.use_issuance_model or self.simulate_heuristic:
    ...
    def update_objective_function(self, model):
        ...
        for a in self.issuance_ages:
            for t in self.time_periods[a:]:
                issuance = model.getVarByName(f"iss[{a},{t}]")
                issuance.Obj = -self.holdingCosts*(a+1) - self.price_pipelines[t-a,0]
        model.update()
    ...
    def create_issuance_lp(self):
        ...
        obj = gb.quicksum(prod[p,l,t] * self.obj_revenue_pipeline[p][l][t] for p in self.products for l in self.production_levels_lp[p] for t in self.time_periods) \
            ...
            - gb.quicksum(iss[a,t]*self.price_pipelines[t-a,0] for a in self.issuance_ages for t in self.time_periods[a:]) \
            - gb.quicksum(iss[a,t]*self.price_means[0] for a in self.issuance_ages for t in self.time_periods[:a]) \
    ...
    def create_heuristic_lp(self):
        ...
        self.obj_revenue_pipeline = {p: {l: {t: self.price_interpolators[p][l](self.price_pipelines[t,p+1]) for t in self.time_periods} for l in self.production_levels_lp[p]} for p in self.products}
        ...
        for t in self.time_periods:
            #outdating
            self.heuristic_model.addLConstr(out[t] >= inv[self.numAges-1,t] - gb.quicksum(iss[self.numAges-1,t] for p in self.products))
            for p in self.products:
    ...
    def simulate_one_step(self, policy=None):
        if policy == None:
            ...
            action = {agent: policy.compute_single_action(self._get_obs()[agent], policy_id = agent, explore=False) for agent in self._agent_ids}
    ...
    def simulate_n_steps(self, nsteps=1, policy=None, plot=False, warm_up=100):
        ...
        while(nsteps > 0):
            ...
            #print(info)
            rewards = np.append(rewards,reward[self._agent_ids[0]])
            purchasing = np.append(purchasing, info["purchase_agent"]["purchasing"])
            revenues = np.append(revenues, info["purchase_agent"]["revenue"])
            purchasing_costs = np.append(purchasing_costs, info["purchase_agent"]["purchasing_cost"])
            holding_costs = np.append(holding_costs, info["purchase_agent"]["holding_cost"])
            prices = np.append(prices, info["purchase_agent"]["prices"])
            for p in self.products:
                production[p] = np.append(production[p], info["purchase_agent"]["production"][p])
            for a in self.ages:
                issuance[a] = np.append(issuance[a], info["purchase_agent"]["issuance"][a])
                inventories[a] = np.append(inventories[a], info["purchase_agent"]["inventory"][a])
            outdating = np.append(outdating, info["purchase_agent"]["outdating"])
    ...
    def simulate_data_for_regression(self, policy=None, data_size=200_000, warm_up_length = 1000):
        ...
        response_matrix = np.concatenate(([info["purchase_agent"]["purchasing"]],info["purchase_agent"]["production"],info["purchase_agent"]["issuance"]))
        ...
        for i in range(data_size):
            ...
            state,reward,_,_,info = self.simulate_one_step(policy)
            reward_array = np.append(reward_array, reward)
            response_matrix = np.vstack((response_matrix,np.concatenate(([info["purchase_agent"]["purchasing"]],info["purchase_agent"]["production"],info["purchase_agent"]["issuance"]))))
    ...
    def simulate_w_cdfs(self, cdfs=None, policy=None, initial_prices = None, initial_inventory=None, warm_up_length = 1000):
        ...
        state = self._get_obs()
        rewards = np.array([])
        feature_matrix = np.concatenate((state["purchase_agent"]["prices"],state["purchase_agent"]["inventory"]))
        state,reward,_,_,info = self.simulate_one_step(policy)
        response_matrix = np.concatenate(([info["purchase_agent"]["purchasing"]],info["purchase_agent"]["production"],info["purchase_agent"]["issuance"]))
        rewards = np.append(rewards, reward["purchase_agent"])
        for _ in range(cdf_length-warm_up_length-1):
            feature_matrix = np.vstack((feature_matrix, np.concatenate((state["purchase_agent"]["prices"],state["purchase_agent"]["inventory"]))))
            state, reward, _, _, info = self.simulate_one_step(policy)
            rewards = np.append(rewards, reward["purchase_agent"])
            response_matrix = np.vstack((response_matrix,np.concatenate(([info["purchase_agent"]["purchasing"]],info["purchase_agent"]["production"],info["purchase_agent"]["issuance"]))))
